...
exports.up = async function (knex) {
  await knex.schema
    ...
    .alterTable("agrements", function (table) {
      table.text("commentaire_correction");
    });

  await knex.schema.raw(`
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'file_category') THEN
        ALTER TYPE front.file_category ADD VALUE 'AGR_ACORRIGER';
      END IF;
    END $$;
  `);
};
...
exports.down = async function (knex) {
  await knex.schema
    .withSchema("front")
    .alterTable("agrements", function (table) {
      table.dropColumn("commentaire_correction");
    });
  await knex.schema.raw(`
    DO $$
    DECLARE
    ...
    BEGIN
      ...
      WHERE t.typname = 'file_category'
        AND n.nspname = 'front'
        AND enumlabel NOT IN ('AGR_ACORRIGER');
