...
const postgres = {
  ...
  ssl: process.env.PGSSLMODE ? {
    ...
    // to authorize CNPG self-signed certificates
  } : false,
...
function requireErrors () {
	...
	errors.ExclusiveParametersError = errors.CronError = void 0;
	class CronError extends Error {
	}
	errors.CronError = CronError;
	...
	class CronTime {
	  ...
	  getNextDateFrom(start, timeZone) {
	    ...
	    while (true) {
	      ...
	      if (!(date.hour in this.hour) && Object.keys(this.hour).length !== 24) {
	        const expectedHour = date.hour === 23 && diff > 864e5 ? 0 : date.hour + 1;
	      ...
	      if (!(date.minute in this.minute) && Object.keys(this.minute).length !== 60) {
	        const expectedMinute = date.minute === 59 && diff > 36e5 ? 0 : date.minute + 1;
	      ...
	      if (!(date.second in this.second) && Object.keys(this.second).length !== 60) {
	        const expectedSecond = date.second === 59 && diff > 6e4 ? 0 : date.second + 1;
	  ...
	  _parseField(value, unit) {
	    ...
	    for (const range of allRanges) {
	      ...
	      if ((match === null || match === void 0 ? void 0 : match[1]) !== void 0) {
	        const [, mLower, mUpper, mStep] = match;
	        let lower = parseInt(mLower, 10);
	        let upper = mUpper !== void 0 ? parseInt(mUpper, 10) : void 0;
	        const wasStepDefined = mStep !== void 0;
	        const step = parseInt(mStep !== null && mStep !== void 0 ? mStep : "1", 10);
	        if (step === 0) {
	          throw new errors_1.CronError(`Field (${unit}) has a step of zero`);
	        }
	        if (upper !== void 0 && lower > upper) {
	          throw new errors_1.CronError(`Field (${unit}) has an invalid range`);
	        }
	        const isOutOfRange = lower < low || upper !== void 0 && upper > high || upper === void 0 && lower > high;
	        if (isOutOfRange) {
...
function requireJob () {
	...
	hasRequiredJob = 1;
	var __awaiter = job && job.__awaiter || function(thisArg, _arguments, P, generator) {
	...
	class CronJob {
	  ...
	  constructor(cronTime, onTick, onComplete, start, timeZone, context, runOnInit, utcOffset, unrefTimeout, waitForCompletion, errorHandler) {
	    ...
	    if (runOnInit) {
	      this.lastExecution = /* @__PURE__ */ new Date();
	  ...
	  _fnWrap(cmd) {
	    ...
	    switch (typeof cmd) {
	      ...
	      case "string": {
	        ...
	        return child_process_1.spawn.bind(void 0, command !== null && command !== void 0 ? command : cmd, args, {});
	      }
	      case "object": {
	        return child_process_1.spawn.bind(void 0, cmd.command, (_a = cmd.args) !== null && _a !== void 0 ? _a : [], (_b = cmd.options) !== null && _b !== void 0 ? _b : {});
	      }
	    }
	  }
...
const createCron = (duration, name, callback) => {
  if (sentry.enabled) {
    const CronJobWithCheckIn = Sentry__namespace.cron.instrumentCron(distExports.CronJob, name);
    return new CronJobWithCheckIn(duration, callback);
  }
  return new distExports.CronJob(duration, callback);
};
...
`;
const formatQuoteElement = (element) => `
<p>
  ${element}
</p>
<BR>
...
`;
const formatArray = ({ headers, lines }) => `
...
`;
var constructMail$1 = (titre, content, signature) => constructMail(titre, content, signature);
...
const smtpSettings = {
  ...
  secure: smtp.secure,
  ...smtp.auth.user && smtp.auth.pass ? { auth: { user: smtp.auth.user, pass: smtp.auth.pass } } : {}
};
const transporterPool = nodemailer__namespace.createTransport(smtpSettings);
const handleTransportEmailPromise = (payload) => new Promise((resolve, reject) => {
...
const transportEmail = async (payload) => {
  try {
...
const transportEmails = async (payloads) => {
...
const initialEmailContent = [
  "<p>Bonjour,</p>",
  "<p>Vous trouverez ci-dessous la liste des d\xE9clarations VAO sur lesquelles une action de votre part est attendue,</p>"
];
const isFUsagerRow = (row) => "mails" in row;
const groupByEmails = (rows) => {
...
const createDSContent = (ds) => {
  const content = [];
  content.push(`<p>${ds.id_fonctionnelle} - ${ds.communes}<br>`);
  content.push(`Statut de la d\xE9claration : ${ds.statut}<br>`);
  content.push(`Date de d\xE9but du s\xE9jour :  ${ds.date_debut}<br></p>`);
  return content;
};
const appendContent = (alerts, starter) => {
  const newContent = [];
  if (alerts.length > 0) {
...
const createContent = ({
  ...
  emailContent.push(
    ...appendContent(
      dsWithAlerts,
      "<p><b>DECLARATIONS NECESSITANT UNE ACTION URGENTE DE VOTRE PART dont la date de d\xE9but de s\xE9jour est \xE0 moins de 10 jours</b>"
    )
  );
  emailContent.push(
    ...appendContent(
...
const generateEmail$1 = (email, content, isBO) => {
  const textDeFin = isBO ? [] : [
    '<p>Si vous avez des difficult\xE9s pour traiter vos d\xE9clarations, vous vous rappelons que vous pouvez <a href="https://vao-assistance.atlassian.net/servicedesk/customer/portals">contacter le support utilisateur</a>.</p>',
    "<p>De plus, vous avez toujours la possibilit\xE9 d\u2019annuler des d\xE9clarations de s\xE9jours qui ne sont plus d\u2019actualit\xE9 pour garder votre tableau \xE0 jour.</p>"
  ];
  const link = isBO ? `${domains.frontBODomain}/sejours` : `${domains.frontUsagersDomain}/demande-sejour/liste`;
  const html = constructMail$1(
    "",
    [
      {
        p: [
      ...
      {
        link,
        text: "Acc\xE9der \xE0 mes d\xE9clarations",
        type: "link"
      }
    ],
    "L'\xE9quipe du SI VAO<br><br><i>Ce courriel est un message automatique, merci de ne pas r\xE9pondre.</i>"
  );
  return {
    from: senderEmail,
    replyTo: senderEmail,
    html,
    subject: `S\xE9jours VAO \u2013 R\xE9capitulatif des d\xE9clarations de s\xE9jour en attente de traitement de votre part`,
    to: [email]
  };
};
const sendEmail = (email, content, isBO) => {
  const mailNew = content.join(";");
  const mail = generateEmail$1(email, mailNew, isBO);
  return mail;
};
const sendEmails = async (rows, isBo) => {
...
const configurableQuery = ({
  ...
  WITH hebergement_exploded AS (
    ...
    ORDER BY
      ...
      ds.date_debut ASC;
`;
const queryRappelDSBo = () => configurableQuery({
  statutsArray: `'${status.TRANSMISE}','${status.EN_COURS}','${status.TRANSMISE_8J}','${status.EN_COURS_8J}'`,
  additionalColumns: "use.mail,",
  additionalJoins: "INNER JOIN geo.territoires ter ON ter.code = ds.departement_suivi INNER JOIN back.users use ON ter.code = use.ter_code",
  additionalGroupBy: "use.mail,",
  additionalOrderBy: ""
});
const queryRappelDSFUsager = () => configurableQuery({
  ...
  additionalJoins: "INNER JOIN front.user_organisme uso ON uso.org_id = ds.organisme_id INNER JOIN front.users use ON use.id = uso.use_id",
  additionalGroupBy: "ds.organisme_id,mailresp,",
  additionalOrderBy: ""
});
const selectRappelDSBo = async () => await pool.query(queryRappelDSBo());
const selectRappelDSFUsager = async () => await pool.query(queryRappelDSFUsager());
const notifyRappelActions = async () => {
...
const cronNotifyRappelActionsBo = () => createCron(
    ...
    [
      {
        p: [
          `
        <p>Bonjour,</p>
        <p>Il vous reste ${deadlineRemind} jours \xE0 compter du ${dateDebutAlerte} pour r\xE9aliser la d\xE9claration de s\xE9jour \xE0 8 jours pour le s\xE9jour \xAB${titre}\xBB.<br>
        Pass\xE9 ce d\xE9lai, il ne vous sera plus possible de constituer le dossier pour votre s\xE9jour.</p>
        `
        ],
        type: "p"
      }
...
const sendRappelDeclarationEmails = async (rows, deadlineRemind) => {
  const mails = rows.map(
    ({ mail, dateDebutAlerte, titre }) => generateEmail({ mail, dateDebutAlerte, deadlineRemind, titre })
  );
  return await transportEmails(mails);
};
...
RETURNING
  id;
`;
const selectRappelDeclarationSejour8j15j = async () => await pool.query(
  querySelectRappelDeclarationSejour8j15j
);
const updateRappel = async (id) => await pool.query(queryUpdateRappel, [id]);
const notifyRappelDeclarationSejourActions = async () => {
...
const cronNotifyRappelDs8j15j = () => createCron(
  notifyDs8jDs15.cron,
  notifyDs8jDs15.name,
...
const queryInsertEvent = `
  ...
  RETURNING
    id as "eventId"
`;
const selectAbandonDeclarations = async () => await pool.query(
  querySelectAbandonDeclarations
);
const insertEvent = async (values) => await pool.query(queryInsertEvent, Object.values(values));
const updateAbandonDeclaration = async (id) => await pool.query(
...
const autoUpdateAbandonDeclarations = async () => {
  ...
  if (listeDSAbandon?.rowCount) {
    return await Promise.all(
      listeDSAbandon.rows.map(
        async ({ statut, id }) => autoUpdateAbandonDeclaration(id, statut)
      )
    );
  }
  return [];
};
const autoUpdateAbandonDeclaration = async (id, statut) => {
  ...
  return await Promise.all([
    insertEvent({
      ...
      bo_user_id: null,
      type,
      type_precision: "Annulation automatique de la d\xE9claration : date de d\xE9but du s\xE9jour d\xE9pass\xE9e",
      metadata: {}
    }),
    insertEvent({
      source,
      ...
      bo_user_id: null,
      type,
      type_precision: `Statut pr\xE9c\xE9dent l'annulation : ${statut}`,
      metadata: {}
    }),
    updateAbandonDeclaration(id)
  ]);
};
const updateStatusDsAction = async () => {
  logger.info(`${updateStatusDs.name} - Start`);
  const startDate = /* @__PURE__ */ new Date();
  try {
    const responses = await Promise.all([
      autoUpdateAbandonDeclarations(),
      pool.query(queryUpdateEnCoursDeclarations),
      pool.query(queryUpdateTermineDeclarations)
    ]);
    const endDate = /* @__PURE__ */ new Date();
    await insertCron({
...
const cronUpdateStatusDs = () => createCron(updateStatusDs.cron, updateStatusDs.name, updateStatusDsAction);
