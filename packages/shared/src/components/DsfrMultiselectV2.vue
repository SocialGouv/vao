...
const { collapse, collapsing, cssExpanded, doExpand, onTransitionEnd } =
  useCollapsable();
...
function handleClickOutside(event: MouseEvent) {
  ...
  if (
    !host.value?.$el.contains(element) &&
    !collapse.value?.contains(element)
  ) {
    close();
  }
}
...
<template>
  ...
  >
    <label :class="finalLabelClass" :for="id">
      ...
      <span v-if="props.hint || $slots.hint?.()" class="fr-hint-text">
    ...
    >
      <p :id="`${id}-text-hint`" class="fr-sr-only">
        Utilisez la tabulation (ou les touches flèches) pour naviguer dans la
