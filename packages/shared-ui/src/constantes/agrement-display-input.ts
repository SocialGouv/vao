    ...
    fields: [
      ...
      },
      {
        ...
        format: ((element: {
          ...
          `${element["adresse"]} ${element["codePostal"]} ${element["commune"]}`) as FieldFormat,
        display: "function",
      },
      {
        label: "Autorisé à la déclaration",
        value: "enabled",
        format: ((element: { enabled: boolean }) =>
          element.enabled ? "Oui" : "Non") as FieldFormat,
        display: "function",
      },
    ],
    filter: (tab: Array<{ enabled: boolean }>) =>
      tab.filter((e) => e.enabled === true),
  },
};

export const IPersonnePhysique = {
  complet: {
    inputType: InputTypes.RADIO,
    label: "Fiche Complète",
    options: ouiNon,
  ...
  adresseDomicile: {
    ...
    formatter: ((value: { label: string }) => value.label) as Formatter,
  },
  adresseSiege: {
    inputType: InputTypes.TO_FORMAT,
    label: "Adresse Siège",
...
export const IResponsableSejour = {
  nom: {
    inputType: InputTypes.TEXT,
    label: "Nom",
  },
...
export const IHebergement = {
  ...
  coordonnees: {
    inputType: InputTypes.TO_FORMAT,
    label: "Adresse",
    formatter: ((value: { adresse: { label: string } }) =>
      value.adresse.label) as Formatter,
  },
};

export const IAttestation = {
  aCertifie: {
    inputType: InputTypes.RADIO,
    label:
