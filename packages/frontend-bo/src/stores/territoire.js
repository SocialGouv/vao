...
export const useTerritoireStore = defineStore("territoire", {
  ...
  actions: {
    async fetch({ limit, offset, sortBy, sortDirection, search } = {}) {
      log.i("fetch - IN");
      try {
        const { territoiresWithPagination } = await $fetchBackend(
          "/territoire/list",
          {
            method: "GET",
            credentials: "include",
        ...
        if (territoiresWithPagination) {
          this.total = territoiresWithPagination.total;
          this.territoires = territoiresWithPagination.territoires.map(
            (territoire) => ({
