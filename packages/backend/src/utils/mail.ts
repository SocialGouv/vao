...
import { generateTypes } from "../helpers/eigMail";
...
export const MailUtils = {
  bo: {
    action: {
      sendAccountDeletionMail: ({ email }: { email: string }) => {
    ...
    authentication: {
      sendForgottenPassword: ({
        ...
        const link = `${config.frontBODomain}/connexion/reset-mot-de-passe?token=${token}`;
      ...
      sendValidationMail: ({
        ...
        const link = `${config.frontBODomain}/connexion/validation?token=${token}`;
    ...
    declarationSejour: {
      sendDeclarationA8joursNotify: ({
        ...
        departementsSecondaires: string[];
      }) => {
        ...
        const link = `${config.frontBODomain}/sejours?statuts=TRANSMISE`;
      ...
      sendDeclarationCanceled: ({
        ...
        const link = `${config.frontBODomain}/sejours/`;
      ...
      sendDeclarationNotify: ({
        ...
        const html = sendTemplate.getBody(
          ...
          `L'équipe du SI VAO<BR><a href=${config.frontUsagersDomain}>${config.frontUsagersDomain}</a>`,
        );
      ...
      sendMessageNotify: ({
        ...
        const link = `${config.frontBODomain}/sejours/${declaration.id}`;
        const html = sendTemplate.getBody(
      ...
      sendUpdateValide8jours: ({
        ...
        const link = `${config.frontBODomain}/sejours/${declaration.id}/formulaire`;
        const html = sendTemplate.getBody(
          "Portail VAO - Nouveau message",
          [
            {
    ...
    eig: {
      ...
      sendToAutre: ({
        ...
        const link = `${config.frontBODomain}/eig/${eig.id}`;
      ...
      sendToOrganisme: ({
        ...
        const orgName = eig.raisonSociale ?? `${eig?.prenom} ${eig?.nom}`;
        const link = `${config.frontUsagersDomain}/eig/${eig.id}`;
    ...
    newVaoAccount: {
      sendDretsNewAccountValidation: ({
        ...
        const linkListeUsersOva = `${config.frontBODomain}/utilisateurs-ova/liste`;
        const html = sendTemplate.getBody(
  ...
  usagers: {
    ...
    authentication: {
      ...
      sendAccountValided: (email: string) => {
        const link = `${config.frontUsagersDomain}/connexion/`;
        const html = sendTemplate.getBody(
      ...
      sendForgottenPassword: ({
        ...
        const link = `${config.frontUsagersDomain}/connexion/reset-mot-de-passe?token=${token}`;
      ...
      sendValidationMail: ({
        ...
        const link = `${config.frontUsagersDomain}/connexion/validation?token=${token}`;
    ...
    declarationSejour: {
      sendACompleterMail: ({
        ...
        const link = `${config.frontUsagersDomain}/demande-sejour/${declaration.id}`;
      ...
      sendAccuseTransmission2mois: (
        {
          dest,
          cc,
          declaration,
        }: { dest: string; cc: string[]; declaration: any },
        firstSubmit?: boolean,
      ) => {
      ...
      sendCanceledMail: ({
        ...
        const link = `${config.frontUsagersDomain}/demande-sejour/liste`;
    ...
    newVaoAccount: {
      ...
      sendNewAccountBlockedByOrganisme: (email: string, motif: string) => {
        const html = sendTemplate.getBody(
      ...
      sendNewAccountValided: (email: string) => {
        const link = `${config.frontUsagersDomain}/connexion/`;
        const html = sendTemplate.getBody(
      ...
      sendOrganismeNewAccountValidation: ({
        email,
        user,
      }: {
        email: string;
        ...
        const link = `https://annuaire-entreprises.data.gouv.fr/etablissement/${user.userSiret}`;
        const linkListeUsersOva = `${config.frontUsagersDomain}/utilisateurs/liste`;
        const html = sendTemplate.getBody(
      ...
      sendWaitAccountValidationDREETS: (email: string, mailDreets: string) => {
        const html = sendTemplate.getBody(
      ...
      sendWaitAccountValidationOrganisme: (email: string) => {
        const html = sendTemplate.getBody(
          "VAO - confirmation de votre demande d’inscription",
          [
            {
