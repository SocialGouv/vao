...
const query = {
  create: `INSERT INTO front.demande_sejour_message (declaration_id,front_user_id,back_user_id,message, created_at) 
    VALUES ($1,$2,$3,$4,NOW() )
    RETURNING id as id;
    `,
  // delete: (criterias) => [
...
module.exports.post = async (declarationId, userId, message, sender) => {
  log.i("create - IN");

  const response =
    sender === "back"
      ? await pool.query(query.create, [declarationId, null, userId, message])
      : await pool.query(query.create, [declarationId, userId, null, message]);
