...
import checkPermissionBoToDownload from "../../middlewares/checkPermissionBoToDownload";
...
const run = async (req: UserRequest) => {
  ...
  await checkPermissionBoToDownload(req, {} as Response, next);
...
const fileMetaData = {
  ...
  userId: "1",
  uuid: "uuid-1",
} as unknown as Awaited<ReturnType<typeof DocumentService.getFileMetaData>>;

describe("middlewares/checkPermissionBoToDownload", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
