...
import { eigModel, TableWithPagination } from "@vao/shared";
...
const status = [allStatus, eigModel.Statuts.BROUILLON, eigModel.Statuts.ENVOYE];
const typesOption = [
  allStatus,
  ...[
...
const headers = [
  ...
  },
  {
    ...
    format: (value) => (value.types ?? []).map((t) => eig.mapEigToLabel[t]),
  },
  {
    column: "statut",
    text: "Statut",
