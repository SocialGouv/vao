...
async function renewToken() {
  ...
  await $fetch(config.public.backendUrl + "/authentication/email/renew-token", {
    ...
    .then(() => {
      toaster.success({
        titleTag: "h2",
        description: `Un nouvel email de validation a été envoyé sur votre boîte mail.`,
      });
    ...
    .catch((error) => {
      toaster.error({
        titleTag: "h2",
        description: `Une erreur est survenue lors de l'envoi de l'email. Veuillez contacter l'assistance.`,
      });
      log.w("renew", error);
    });
}
