<template>
  <div class="fr-col-10">
    ...
    <dl>
      ...
      <dd>{{ personnePhysique.adresseSiege }}</dd>
      <dt>
        <strong>Adresse de ses activités: </strong>
      </dt>
      <dd>{{ personnePhysique.adresseActivites }}</dd>
    </dl>
  </div>
</template>
...
const { personnePhysique } = storeToRefs(agrementStore);
...
// Fonctions pour passer en mode édition
...
async function validateAndSave() {
  ...
  try {
    ...
    await agrementStore.updatePersonnePhysique(personnePhysique.value.id, {
